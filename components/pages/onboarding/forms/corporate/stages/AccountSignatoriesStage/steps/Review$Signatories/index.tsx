...
import { proofOfIdentityModel$Signatories } from '../ProofOfIdentity$Signatories/model/proofOfIdentity$SignatoriesModel';
import { pepModel$Signatories } from '../Pep/model/pepModel$Signatories';
...
import type { Signatory,Director,BeneficialOwner } from '@/types/forms/corporateSchema';

export const Review$Signatories: FormStep = () => {
    const { formAction, form } = useKYCFormContext();
    const { getValues } = form;

    const signatoriesCount = ( getValues( 'accountSignatories.signatories' ) as Signatory[] ).length; 
    const directorsCount = ( getValues( 'accountSignatories.directors' ) as Director[] ).length;
    const beneficialOwnersCount = ( getValues( 'accountSignatories.beneficialOwners' ) as BeneficialOwner[] ).length;
			...
			<FormContent className='bg-white px-0 pt-0'>
				<div className='bg-neutral-50 space-y-[16px]'>
					...
					<ReviewerSection
                        ...
                        fieldModel={ proofOfIdentityModel$Signatories }
                        accordionTitle={signatoriesTitle}
					/>
					<ReviewerSection
                        ...
                        fieldModel={ pepModel$Signatories }
                        accordionTitle={signatoriesTitle}
					/>
					<ReviewerSection
