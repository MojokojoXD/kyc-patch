...
>(({ className, ...props }, ref) => {
	...
	return (
		<p
			...
			className={cn('text-sm text-neutral-500 dark:text-slate-400', className)}
			{...props}
		/>
	);
});
...
const FormMessage = React.forwardRef<
	HTMLParagraphElement,
	React.HTMLAttributes<HTMLParagraphElement> & { position?: 'relative' | 'absolute' }
>(({ className, children,position = 'absolute', ...props }, ref) => {
	...
	return (
		<p
			...
			id={formMessageId}
			className={cn(
				`text-sm font-normal text-error-500 dark:text-red-900 flex items-center ${position + ' pb-1.5'}`,
                className
			)}
			{...props}>
